...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        Order = Orders(items_json = item_json,name=name,amount=amount,email=email,
                       address1=address1,address2=address2,city=city,state=state,
                       zip_code = zip_code,phone=phone)
        print(amount)
        Order.save()
        update = OrderUpdate(order_id = Order.order_id,update_desc = "the odrder has been placed")
        update.save()
        thank = True
        id = Order.order_id
        oid = str(id) + "AISPACEX"
